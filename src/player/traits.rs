...
pub trait Player {
    ...
    fn replace_card(&mut self, to_replace: usize, card: Identity) -> Identity {
        let mut hand = self.get_hand();
        let replaced_card = hand[to_replace].clone();
    ...
    fn take_card(&mut self, state: &GameState, card: Identity) -> Option<Identity> {
        if self.hand_full() {
            if let Some(to_replace) = self.choose_card_to_replace(state, &card) {
                return Some(self.replace_card(to_replace, card));
            } else {
                return Some(card);
            }
        } else {
    ...
    fn take_cards(&mut self, state:&GameState, cards: Vec<Identity>) -> Vec<Identity> {
	let mut to_drop = Vec::new();
	for card in cards {
	    self.take_card(state, card.clone());
	}
	return to_drop;
    }
